...
import {
  ButtonConfig,
  FormFieldMapping,
} from '../../models/form-field-mapping.model'; // Ajusta la ruta
  ...
  implements OnInit, OnChanges, AfterViewInit, OnDestroy
{
  ...
  ngOnChanges(changes: SimpleChanges): void {
    ...
    if (
      ...
      (changes['cssContentString'] || changes['contentId']) &&
      this.viewInitialized
    ) {
      this.injectCss();
    }
  ...
  private initializeInjectedContentInteractions(): void {
    ...
    const setupLogic = (observerInstance?: MutationObserver) => {
      if (
        this.htmlContainerRef?.nativeElement &&
        this.htmlContainerRef.nativeElement.childElementCount > 0
      ) {
        if (
          this.isForm &&
          this.dynamicForm &&
          this.formMappings &&
          this.formMappings.length > 0
        ) {
    ...
    if (this.htmlContainerRef?.nativeElement) {
      if (!setupLogic()) {
        ...
        observer.observe(this.htmlContainerRef.nativeElement, {
          childList: true,
          subtree: true,
        });
      ...
      console.warn(
        `DynamicViewer (${this.contentId}): htmlContainerRef no disponible para inicializar interacciones.`,
      );
    }
  }
  ...
  private setupInjectedActionClickListeners(): void {
    ...
    if (!this.htmlContainerRef?.nativeElement) {
      console.warn(
        `DynamicViewer (${this.contentId}): htmlContainerRef no disponible para configurar ActionClickListeners.`,
      );
      return;
    }

    const hostElement = this.htmlContainerRef.nativeElement;
    const unlistenClickFn = this.renderer.listen(
      hostElement,
      'click',
      (event: Event) => {
        ...
        while (currentElement && currentElement !== hostElement && depth < 10) {
          if (
            ...
            currentElement.matches('button[data-dynamic-action]')
          ) {
            ...
            if (action) {
              ...
              console.warn(
                `DynamicViewer (${this.contentId}): Botón encontrado con data-dynamic-action pero el atributo está vacío.`,
              );
            }
          ...
          if (
            ...
            currentElement.parentElement === hostElement.parentElement
          ) {
            break;
          }
  ...
  private setupInjectedFormSubmitPrevention(): void {
    ...
    const formsInInjectedHtml =
      this.htmlContainerRef.nativeElement.querySelectorAll('form');
          ...
          switch (config.type.toLowerCase()) {
            ...
            case 'minlength':
              if (config.value !== undefined)
                controlValidators.push(Validators.minLength(config.value));
              break;
            case 'maxlength':
              if (config.value !== undefined)
                controlValidators.push(Validators.maxLength(config.value));
              break;
            case 'pattern':
              if (config.value)
                controlValidators.push(Validators.pattern(config.value));
              break;
            case 'min':
              if (config.value !== undefined)
                controlValidators.push(Validators.min(config.value));
              break;
            case 'max':
              if (config.value !== undefined)
                controlValidators.push(Validators.max(config.value));
              break;
            case 'requiredtrue':
            ...
            case 'matchValue':
              if (config.value) {
                controlValidators.push((control: AbstractControl) => {
                  return control.value === config.value
                    ? null
                    : { matchValue: { valid: false, value: config.value } };
                });
              }
              break;
            default:
              console.warn(`Validador no reconocido: ${config.type}`);
          }
        });
      }
  ...
  private cacheButtonElements(): void {
    ...
    if (this.buttonConfigs && this.htmlContainerRef?.nativeElement) {
      this.buttonConfigs.forEach((config) => {
        ...
        if (!element) {
          console.warn(
            `DynamicViewer (${this.contentId}): Botón con selector "${config.selector}" no encontrado.`,
          );
        }
      });
    }
  ...
  private subscribeAndSetErrorVisualsOnInputs(): void {
    if (
      ...
      console.warn(
        'DynamicViewer: No se pueden configurar visuales de error. Formulario, mapeos o contenedor no listos.',
      );
      return;
    }
    ...
    this.formMappings.forEach((mapping) => {
      ...
      if (control && inputElement) {
        ...
        if (!control) {
          console.warn(
            `Visuals: FormControl no encontrado para "${mapping.controlName}"`,
          );
        }
        if (!inputElement) {
          console.warn(
            `Visuals: Elemento DOM (input) no encontrado con selector "${mapping.domSelector}" para control "${mapping.controlName}"`,
          );
        }
      }
    });
  ...
  triggerSubmit(): void {
    if (!this.dynamicForm) {
      console.warn(
        `DynamicViewer (${this.contentId}): triggerSubmit llamado pero dynamicForm no está definido.`,
      );
      return;
    }
    ...
    if (this.dynamicForm.valid) {
      ...
      console.warn(
        `DynamicViewer (${this.contentId}): Formulario Angular INVÁLIDO.`,
      );
    }
  }
  ...
  public disableFormField(controlName: string): void {
    if (this.dynamicForm && this.dynamicForm.get(controlName)) {
      ...
      console.warn(
        `DynamicViewer (<span class="math-inline">\{this\.contentId\}\)\: No se pudo deshabilitar FormControl\: "</span>{controlName}"`,
      );
    }
  }
  ...
  public enableFormField(controlName: string): void {
    if (this.dynamicForm && this.dynamicForm.get(controlName)) {
      ...
      console.warn(
        `DynamicViewer (<span class="math-inline">\{this\.contentId\}\)\: No se pudo habilitar FormControl\: "</span>{controlName}"`,
      );
    }
  }
  ...
  public disableElementBySelector(selector: string): void {
    if (this.htmlContainerRef?.nativeElement) {
      ...
      if (element) {
        ...
        console.warn(
          `DynamicViewer (<span class="math-inline">\{this\.contentId\}\)\: Elemento no encontrado para deshabilitar\: \[</span>{selector}]`,
        );
      }
    }
  }
  ...
  public enableElementBySelector(selector: string): void {
    if (this.htmlContainerRef?.nativeElement) {
      ...
      if (element) {
        ...
        console.warn(
          `DynamicViewer (<span class="math-inline">\{this\.contentId\}\)\: Elemento no encontrado para habilitar\: \[</span>{selector}]`,
        );
      }
    }
  }
