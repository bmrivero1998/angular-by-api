import { ButtonConfig, FormFieldMapping } from '../models/form-field-mapping.model'; // Ajusta la ruta si es necesario
...
export const MockApiResponseData: { ok: boolean; doc: MockApiItem[] } = {
  ...
  doc: [
      ...
      formMappings: [
        {
          ...
          validatorConfig: [
            ...
            { type: 'pattern', value: '^[0-9]{5}$', message: 'Debe ser un código postal de 5 dígitos.' },
          ],
        },
      ],
      formInitialData: {},
    ...
    },
    {
      ...
      formMappings: [
        { controlName: 'fullName', domSelector: '#fullNameInput', eventType: 'input', errorDisplaySelector: '#fullNameInput-error', defaultValue: '', validatorConfig: [{ type: 'required', message: 'El nombre completo es requerido.' }, { type: 'minLength', value: 3, message: 'El nombre debe tener al menos 3 caracteres.' }] },
        { controlName: 'email', domSelector: '#emailInput', eventType: 'input', errorDisplaySelector: '#emailInput-error', defaultValue: '', validatorConfig: [{ type: 'required', message: 'El correo es requerido.' }, { type: 'email', message: 'Formato de correo inválido.' }] },
        { controlName: 'password', domSelector: '#passwordInput', eventType: 'input', errorDisplaySelector: '#passwordInput-error', defaultValue: '', validatorConfig: [{ type: 'required', message: 'La contraseña es requerida.' }, { type: 'minLength', value: 6, message: 'La contraseña debe tener al menos 6 caracteres.' }] },
        { controlName: 'confirmPassword', domSelector: '#confirmPasswordInput', eventType: 'input', errorDisplaySelector: '#confirmPasswordInput-error', defaultValue: '', validatorConfig: [{ type: 'required', message: 'Confirma tu contraseña.' }] /* Podrías añadir un validador a nivel de FormGroup para 'matchPassword' */ },
        { controlName: 'country', domSelector: '#countrySelect', eventType: 'change', errorDisplaySelector: '#countrySelect-error', defaultValue: '', validatorConfig: [{ type: 'required', message: 'Selecciona un país.' }] },
        { controlName: 'gender', domSelector: 'input[name="gender"]', eventType: 'change', errorDisplaySelector: '#gender-error', defaultValue: null /* o '' */ }, // Para radios, el selector puede ser por 'name'
        { controlName: 'comments', domSelector: '#commentsTextarea', eventType: 'input', errorDisplaySelector: '#commentsTextarea-error', defaultValue: '', validatorConfig: [{ type: 'maxLength', value: 200, message: 'Máximo 200 caracteres.' }] },
        { controlName: 'acceptTerms', domSelector: '#termsCheckbox', eventType: 'change', errorDisplaySelector: '#termsCheckbox-error', defaultValue: false, validatorConfig: [{ type: 'requiredTrue', message: 'Debes aceptar los términos.' }] } // Para checkboxes 'required' significa 'requiredTrue'
      ],
      formInitialData: {
      },
      buttonsConfig: [
