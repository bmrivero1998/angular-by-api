...
import { ApiDrivenContent, DynamicClickPayload } from "./interfaces/DynamicContent.interface";
...
export interface DisplayableInAppComponent extends ApiDrivenContent { // Hereda de ApiDrivenContent
  safeHtml: SafeHtml;
}
...
export class AppComponent implements OnInit, OnDestroy {
  ...
  private parentFormValidation(): void {
    ...
    this.parentForm.valueChanges.subscribe(() => {
      const userRegistrationForm = this.parentForm.get('userRegistration') as FormGroup;
      if (userRegistrationForm) {
        const passwordControl = userRegistrationForm.get('password'); // Es 'password', no 'passwordValue'
        const confirmPasswordControl = userRegistrationForm.get('confirmPassword');
        ...
        if (passwordControl && confirmPasswordControl) { 
          if (passwordControl.value === confirmPasswordControl.value) {
            ...
            if (!confirmPasswordControl.hasError('passwordMismatch')) {
               confirmPasswordControl.setErrors({ ...confirmPasswordControl.errors, passwordMismatch: true }, { emitEvent: false });
            }
          }
        }
      ...
      const postalCodeForm = this.parentForm.get('lookupPostalCodeForm') as FormGroup;
    if (postalCodeForm) {
  ...
  handleViewerFormSubmission(payload: { formId?: string, data: any }): void {
    console.log(`AppComponent: Formulario ${payload.formId} enviado desde viewer con datos:`, payload.data);
  }

  handleViewerActionClick(payload: DynamicClickPayload): void {
    
    if (payload.action === 'clearRegistrationForm' && payload.sourceId === 'userRegistration') {
